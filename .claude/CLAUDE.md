...
- `OcclusionFade` is runtime-added by `ResourceNode.Start` (every type) and `OcclusionFade.AttachTo` in Hut / Watchtower / Workshop / Shipyard `Start`; **it retires itself on first measure below `MinSilhouetteHeight` 1.6**, which is what keeps bushes, low rocks and the campfire out — never gate it by type. The manager's test is screen-space at 20 Hz (two body points per unit, `UnitHalfWidth` 0.22, `DepthMargin` 0.6, `ReleaseSlack` 1.2 hysteresis), not a raycast; **tightness is per object** (`silhouetteTightness` 0.42 canopy, 0.9 building) because a canopy is gaps and a hut is a box. Fading out is faster than fading back on purpose. `SetSurface` (opaque↔transparent keywords, `renderQueue` back to -1) runs only when the fade crosses full opacity. Walls are deliberately excluded (a line of ghosting cells reads worse than the problem) and a faded object still casts a solid shadow — both wait on the cutout shader.
- **An occluder's depth is read AT THE COVER POINT, never at its nearest end (2026-09-08):** `SqrDistanceToSegment` hands back the parameter `t` of the closest point on the screen-space base→top segment and the depth compare uses `Lerp(baseScreen.z, topScreen.z, t)`. The tilted ortho camera puts a tall palm's crown metres nearer than its trunk, so a `Min` of the two endpoints faded the tree for colonists standing well in FRONT of it — the error grows with height, which is why palms broke and huts did not.
- **`IMaterialSet` is how a second system finds an object's existing material instances** (2026-09-08): `ResourceNode`, `Workshop` and `Shipyard` implement it, `OcclusionFade` asks for it and only collects its own when nobody answers. Reading `.materials` twice on one object is the "hover glow and fade fight" bug.
