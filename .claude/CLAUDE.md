...
| Right-click | Smart command for your character: fetch a pickup / hand-harvest a tree, bush or rock / deposit at the campfire (and work its queue) / work a Workshop bench / walk there |
...
- **Ground pickups glow at rest, not only on hover.** A stick in grass at RTS zoom is invisible. Idle 0.6 / hover 2.8, collected in `Start` (not `Awake`) because scatter salvage and the wreck cargo mount their art as a child in the frame the object is created.
