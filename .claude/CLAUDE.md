...
**Pending playtest:** everything in `DevQuests.txt` (Slices 3–6 need Setup Everything first for the archer body and the Shipyard assets; utility colonists, priorities, Information, the dev quests themselves, warrior stances / attack slots and the gear-up trip are 09-07). `DevQuests` is editor/dev-build only (`const Enabled`, `#pragma 0162`); quest ids are batch-slug + index, so append rather than insert. A raid tuning pass is pending.
