...
Repo root: `docs/` (PHASE_HISTORY, CONTROLS_AND_CHECKLIST, SIMULATION, MENU_WIREFRAMES), `*_PLAN.md` design docs (ARCHITECTURE_LAP = the current plan; TERRAIN_SYSTEM, PHASE_10_VISUAL_OVERHAUL, CRAFTING_AND_PLAYER_CHARACTER, RESEARCH_AND_DAYS, COLONY_EXPANSION — source of truth for locked decisions), `SimSweeps/` + `tools/run-sim.ps1`, and `islandrts/`.
...
**Pending playtest:** everything in `DevQuests.txt` — 27 batches, about two thirds auto-tick — and `Playtests/` is still empty, so the "Auto quests" + "Dev quests" batches (the loop itself) are played FIRST. Ship blockers A1/A2/A3/A5 are closed, A4 is one release build to play. `DevQuests` is editor/dev-build only (`const Enabled`, `#pragma 0162`); quest ids are batch-slug + index, so append rather than insert. A raid tuning pass is pending.

**Next: THE ARCHITECTURE LAP (decided 2026-09-09, freeze lifted): Factions → spatial hash + AI LOD → colony governor (+ diplomacy, dock trade) → save/load (dawn checkpoint) → archipelago islands.** `ARCHITECTURE_LAP_PLAN.md` at the repo root is the plan: per-step shape, migration commits, definitions of done, the 2026-09-09 decisions and the open ones. Rules while it runs: every step green in all four Roslyn configs + a sim regression (Turtle/Rush/Eco n=6 against `SimSweeps/baseline-2026-09-09/`, taken BEFORE step 1) before commit; a compatibility shim lives one step then dies and its accessor joins the banned list; **faction is read in `Start` or later, never `Awake`** (`Instantiate` runs Awake before the owner can be set); one data model (`ColonyLedger`) for the abstract tier, the save file and island travel. Then `ALPHA_PLAN.md` B → G (playtest debt → tuning → tutorial → feedback form → zip) on the new foundation. A bug found in playtest gets fixed, a wish gets written down. Parked: the Story tab, weather part two, `COLONY_EXPANSION_PLAN.md`, Phase 10 Stages 3–4, the Warehouse, building upgrades, shore-wading enemies.
