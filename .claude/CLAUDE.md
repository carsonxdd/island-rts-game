...
| Script | Purpose |
|--------|---------|
| `GameManager.cs` | Victory (the dawn after day 30 — the rescue ship) / defeat (campfire lost), statistics |
| `RaidDirector.cs` | Rolls at each dawn whether raiders land tonight (escalating chance, guaranteed after 5 quiet nights, none before day 3) and how many (day + colony prosperity); runtime-added to the `EnemySpawner` object, which now only lands what it is told |
...
- **Resource-node click hitboxes live on layer 8 `Nodes` (`ResourceNode.ClickLayer`), never Default (2026-09-03).** Same bug as the ghost collider, wearing bark: node boxes on Default were hit by the placement ray instead of the terrain, so the ghost parked at the base of any tree the cursor crossed. `ResourceNode.Awake` sets the root layer at runtime (prefabs and `PropScatter` nodes alike; hover/click still work because `OnMouse*` uses the camera's eventMask, not the ground mask). `PropScatter.SpawnNode` also sizes the box from the mesh bounds in the model's LOCAL frame with a trunk-sized footprint (`clamp(canopy × 0.35, 0.9, 1.6)`) — the world AABB of a yawed palm was a 4 m rectangle. When Slice D adds node commands to the player, `PlayerCharacter.CommandMask` must include `ResourceNode.ClickMask`.
...
### Day/Night & Combat
- Day: 100s, Night: 50s (scene values on `DayNightCycle`; was 120/60 until 2026-09-02). Clock speed is phase-based — day and night each cover half the 0..1 time parameter at their own rate. (Fixed 2026-08-25: the old constant `timeSpeed = 1/(day+night)` made both phases actually last 90s and silently ignored the configured lengths.)
- **The run is a 30-day calendar, not nightly waves (2026-09-02).** `RaidDirector` rolls at each DAWN whether raiders land tonight: `chance = (0.15 + 0.2 × quietNights) × Difficulty.raidFrequency`, never before day 3, guaranteed after 5 quiet nights. The HUD calendar chip and a banner announce it for the whole day. Size is fixed at roll time: `round((2 + 0.4 × day + 0.08 × prosperity) × Difficulty.enemyCount)`, where prosperity counts colonists, warriors, buildings and stock on hand (`RaidDirector.Prosperity`). `EnemySpawner.SpawnRaid(count)` only lands what it is told.
- Victory: reach the dawn after day `daysToSurvive` (Normal 30; Peaceful/Relaxed 20 — length is deliberately NOT a difficulty lever, Hard/Brutal raid more often instead)
...
- **`FitPanelHeight` must force a FULL recursive layout rebuild, not just the outer group's two `CalculateLayoutInput` calls (2026-09-03).** A nested `VerticalLayoutGroup` (the campfire panel's tab bodies) reports its preferred height from its OWN `CalculateLayoutInputVertical`, which the outer call never runs — so the outer group summed the bodies at zero, the panel came out ~130px, and `childControlHeight` squished every row inside to fit ("campfire panel squished with no spacing"). It uses `LayoutRebuilder.ForceRebuildLayoutImmediate` now. Any code that measures a layout group before the first layout pass has the same trap.
...
- **Difficulty must never leak into a balance sweep.** `Difficulty.Active` returns the Normal preset whenever `SimHooks.Simulating`, or a difficulty saved in the developer's PlayerPrefs would silently skew every simulated run. `GameManager.Start` additionally guards `daysToSurvive` behind `!SimHooks.Simulating` — the sweep sets that from `sceneLoaded`, which runs *before* Start, so an unguarded assignment would clobber the config's value back to the preset's.
...
- **A wheel notch over a scrolling list belongs to that list — and the test is a ScrollRect, never `IsPointerOverGameObject` (2026-09-03).** uGUI consumes the wheel for a `ScrollRect` under the cursor, but `CameraController.UpdateZoom` reads `Input.GetAxis("Mouse ScrollWheel")` straight from the device, so scrolling the crafting list zoomed the world out from under it. The first fix asked `EventSystem.current.IsPointerOverGameObject()` and **broke zoom entirely**: the HUD strip and the resource bar's hit surfaces are raycastable across big parts of the screen, so that answer is true almost everywhere. `CameraController.PointerOverScrollView` does an `EventSystem.RaycastAll` into a reused list and blocks only when a hit has a `ScrollRect` above it, and it runs only on frames the wheel actually moved. `PauseController.BlockGameplayInput` does not cover any of this — a gameplay panel like the campfire is not a menu.
- **Every scrolling list gets a scrollbar and 60 units of wheel sensitivity.** `MenuBuilder.ScrollColumn` builds a slim permanent bar down the viewport's right edge (`ScrollbarWidth`, reserved out of the content's right padding so no row hides under it) and its handle is an **opaque white** Image, because a `ColorBlock` MULTIPLIES the graphic's colour — the same rule the HUD chips follow. Visibility is `Permanent`, never `AutoHideAndExpandViewport`: that mode resizes the viewport, which fights the `LayoutElement` sizing it.
...
| Right-click | Smart command for your character: fetch a pickup / pick a bush by hand, or work a tree, rock or ore boulder once its tool is crafted / deposit at the campfire (and work its queue) / work a Workshop bench / walk there |
...
- Day/night cycle on a 30-day calendar; raids rolled at dawn, announced all day, sized by day + prosperity
- Victory (the dawn after day 30) / defeat (campfire destroyed)
