...
public static class FullSetup
{
    ...
    [MenuItem("Tools/Founding Tide/Setup Everything (In Order)", false, -100)]
    ...
    {
        EditorUtility.DisplayProgressBar("Founding Tide setup", $"{label}…", (float)index / total);
    }
    ...
    /// </summary>
    [MenuItem("Tools/Founding Tide/Open Game Scene (MainIsland)", false, -99)]
    ...
    [MenuItem("Tools/Founding Tide/Open Menu Scene (MainMenu)", false, -98)]
    public static void OpenMenuSceneMenu()
    {
        ...
        Debug.LogError($"[Setup] {MenuScenePath} does not exist yet — run " +
                       "Tools > Founding Tide > Menus > Setup Main Menu Scene (or Setup Everything).");
    }
}
