...
namespace FoundingTide.ArtGen
{
    ...
    {
        private const string MenuRoot = "Tools/Founding Tide/Low-Poly Templates/";
        ...
        private static readonly Plumb[] Buildings =
        {
            new Plumb
            {
                // Housing tier 1 (2026-09-18). Same 2 x 2 footprint and carve volume as the
                // Hut it upgrades into - only the height differs, so the health bar sits
                // lower and the pad is interchangeable between the tiers.
                // Tent.prefab is BORN HERE, as a copy of the Hut it upgrades into, so it
                // inherits the Hut + HealthBar components on the root; NewContentSetup then
                // retunes the housing numbers (1 bed, 50 HP). Without CopyFrom this entry
                // pointed at a prefab only step 6 creates, and step 2 died on it.
                // TentData.asset is step 6's too (it sets placementHeight itself), so this
                // entry deliberately names no BuildingData - it does not exist yet at step 2.
                GameplayPrefab = "Assets/Prefabs/Tent.prefab",
                CopyFrom       = "Assets/Prefabs/Hut.prefab",
                ArtPrefab      = ArtPrefabRoot + "Buildings/Tent.prefab",
                ResetRootScale = true, StripRootMesh = true,
                BoxSize = new Vector3(2f, 1.6f, 2f), BoxCenter = new Vector3(0f, 0.8f, 0f),
                ObstacleExtents = new Vector3(1.1f, 0.8f, 1.1f), ObstacleCenter = new Vector3(0f, 0.8f, 0f),
                BarOffset = 2.1f, BarWidth = 2f, BarHeight = 0.2f
            },
            new Plumb
            {
        ...
        private static bool PlumbPrefab(Plumb p, StringBuilder summary)
        {
            ...
            // LoadPrefabContents THROWS on a missing path, it does not return null, so the
            // check has to happen before the call. One table entry pointing at a prefab that
            // does not exist yet must never abort Setup Everything's remaining steps
            // (2026-09-18: the Tent's entry took steps 3-8 down with it).
            if (AssetDatabase.LoadAssetAtPath<GameObject>(p.GameplayPrefab) == null)
            {
                Debug.LogWarning("[LowPoly] Gameplay prefab not found, skipping: " + p.GameplayPrefab);
                return false;
            }
