...
/// </remarks>
public class BaseBuilding : MonoBehaviour, ITargetable, IHousing, IDropoff
{
    ...
    /// room (Storage Pits, Racks and Baskets) and every standing Storehouse
    /// adds <see cref="Storehouse.RoomBonus"/> (2026-09-16). Read live, never
    /// cached — the same rule every other upgrade follows.
    /// </summary>
    ...
    public int StockpileCapacity => BaseStockpileCapacity + Faction.Knowledge.StockpileRoom
        + TargetingUtil.CountOwned(Storehouse.ActiveList, Faction) * Storehouse.RoomBonus;

    /// <summary>The fire still stands (IDropoff). Housing has its own flag; this is the body.</summary>
    public bool IsAlive => healthComponent == null || healthComponent.IsAlive;
    /// <summary>The collider a returner measures its edge distance against (IDropoff).</summary>
    public Collider ApproachCollider => housingCollider != null ? housingCollider : GetComponent<Collider>();
    ...
    public const int DropoffSlotCount = DropoffRing.SlotCount;
    private DropoffRing dropoffRing;
    private DropoffRing Ring => dropoffRing ?? (dropoffRing = new DropoffRing(transform, () => ApproachCollider));
    ...
    /// The ring itself is <see cref="DropoffRing"/> since the Storehouse (2026-09-16).
    /// </summary>
    ...
    /// <summary>Walkable point at the fire's collider edge along the slot's bearing (carve-safe).</summary>
