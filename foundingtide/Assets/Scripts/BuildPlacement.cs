...
public class BuildPlacement : MonoBehaviour
{
    ...
    // The Tent is housing tier 1 and the cheapest thing in the palette (2026-09-18),
    // so it is what build mode opens on. The Hut is not placeable at all any more.
    public BuildingType selectedBuildingType = BuildingType.Tent;
    ...
    void Update()
    {
        ...
        if (isPlacing && currentGhost != null)
        {
            ...
            if (KeyBindings.Down(KeyBindings.Action.SelectTent)) SelectBuilding(BuildingType.Tent);
    ...
    /// <summary>
    /// Switch to a different building type during placement mode. Public since
    /// 2026-09-18: the build palette's tiles call it, same as the number keys.
    /// </summary>
    public void SelectBuilding(BuildingType type)
    {
        ...
        // A tier reached only by upgrading (the Hut) is never placed from the palette.
        if (!data.placeable)
        {
            Debug.LogWarning($"BuildPlacement: {type} is not placeable - it is an upgrade of another building.");
            return;
        }

        // The research gate lives on the asset now (2026-09-18). Still one Knowledge.Has
        // at the site that decides; the palette reads the same field to grey its tile.
        if (!data.UnlockedFor(Factions.Player))
        {
            if (PlayerCharacter.Instance != null)
                PlayerCharacter.Instance.SetActivity($"Research {data.RequiredResearchTitle} to build a {data.buildingName}", 3f);
            return;
        }
