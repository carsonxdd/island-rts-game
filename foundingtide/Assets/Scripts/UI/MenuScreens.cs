...
public class MenuScreens : MonoBehaviour
{
    ...
    private void BuildMain()
    {
        ...
        // One line, auto-sized down to fit. "FOUNDING TIDE" at the full TitleSize
        // is wider than MenuWidth, and how much wider depends on the font's own
        // metrics, so let TMP shrink it rather than hard-coding a point size that
        // the next font swap would overflow. NoWrap because the answer to "too
        // wide" here must be smaller text, never a second line.
        TextMeshProUGUI title = MenuBuilder.Label(col.transform, "FOUNDING TIDE",
            MenuStyle.TitleSize, MenuStyle.TextAccent);
        title.textWrappingMode = TextWrappingModes.NoWrap;
        title.enableAutoSizing = true;
        title.fontSizeMax = MenuStyle.TitleSize;
        title.fontSizeMin = 30f;
        title.characterSpacing = 6f;
        title.gameObject.AddComponent<LayoutElement>().preferredHeight = 84f;
        ...
        // (0.3.0-alpha.1, alpha.2, ...) and the same string every playtest and
