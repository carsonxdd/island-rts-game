...
public class PlayerCharacter : UnitBase<PlayerCharacter>
{
    ...
    void Start()
    {
        ...
        Minimap.Ensure();
        BuildPaletteHUD.Ensure();       // shows itself in build mode (2026-09-18)
        SelectedBuildingHUD.Ensure();   // shows itself when a building is clicked
    ...
    {
        if (PointerBlock.OverHud) return;   // a click on the map moves the camera, not the castaway
