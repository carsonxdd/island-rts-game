...
///  - it does not cover fogged ground: the material multiplies the line alpha by the
///    fog mask's explored channel, so the grid cannot show you an island you have not
///    walked (see <see cref="GridShader"/>);
...
public class GridOverlay : MonoBehaviour
{
    ...
    void Build()
    {
        ...
        _mat = new Material(GridShader());
        // Named, not Material.color: the grid shader's own property is _Color, and a
        // fallback sprite material has one too.
        _mat.SetColor("_Color", lineColor);
    ...
    /// <summary>
    /// The grid's own shader: a transparent line shader whose alpha is multiplied by the
    /// fog mask's EXPLORED channel, so the overlay only exists on ground the colony has
    /// seen. Without it the grid drew every buildable cell on the island and leaked the
    /// shape of the map straight through the fog (2026-09-19).
    ///
    /// The mask is read per fragment, so the mesh is still built once per toggle and the
    /// lines fade in with the terrain as a scout reveals ground. Explored, not visible:
    /// placement allows explored-but-unwatched ground, so the grid shows exactly where a
    /// building may go.
    ///
    /// Falls back to the old sprite shader if the asset is missing (an un-set-up project,
    /// or the showcase scene) - the grid is then unfogged, which is what those scenes want.
    /// </summary>
    static Shader GridShader()
    {
        Shader s = Shader.Find("FoundingTide/GridOverlay");
        if (s == null) s = Resources.Load<Shader>("Shaders/GridOverlay");
        if (s == null)
        {
            Debug.LogWarning("GridOverlay: Resources/Shaders/GridOverlay missing - the grid will ignore the fog of war.");
