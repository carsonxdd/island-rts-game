...
public static class RivalFounder
{
    ...
    /// <summary>
    /// One tent on the first clear spot of a four-lap ring, so the camp can grow past the
    /// fire's three beds. True when one was placed. A Tent since 2026-09-18 - a rival opens
    /// on the same tier-1 shelter the player does, and its governor tents up from there.
    /// </summary>
    static bool PlaceHut(Faction rival, Vector3 site, TerrainGrid tg)
    {
        BuildingData hutData = BuildingDatabase.Instance != null
            ? BuildingDatabase.Instance.GetBuildingData(BuildingType.Tent) : null;
