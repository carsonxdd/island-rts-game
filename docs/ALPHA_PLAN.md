...
**Progress (2026-09-09):** steps 1-4 are built and compile, unplaytested — `FogOfWar` grid (2 m, explored + visible, eased RG16 mask), `VisionSource` on every unit and building (12 / 14 / 18 / 10 / 35 m), fog in the `OccluderCutout` and water shaders with fog-aware terrain and decor via `FogMaterials`, and `FogVisibility` hiding raiders (while unseen) and nodes and pickups (until explored). The F4 menu has a "Reveal map" toggle. The pickup half of the resource gate landed the same day (both pickup scans skip unexplored ground, found while fixing the forage stutter). **Step 5 landed later the same day:** `ResourceAvailability` skips nodes on unexplored ground (scan time only; a worker at a node is standing on explored ground), `GuardStance.Allows` — the one filter behind Engage — refuses a raider on ground nothing is watching, and `GhostPlacer` / `WallLinePlacer.CellBlocked` (mirrored in `SimBuilder`) refuse unexplored cells. **Step 6 landed 2026-09-09:** `Minimap` — top-right, north-up, one texture at two texels per fog cell painted from the terrain's surface materials through the explored/visible grid, then buildings, walls, colonists, warriors, the castaway, raiders on watched ground, the camera footprint, and a pulse where a raid landed (`EnemySpawner.OnRaidLanded`, fog or not). Left-click or drag centres the camera; `Minimap.PointerOver` refuses the gameplay clicks under it. Section I is built end to end; the whole of it is unplaytested as one thing. Left: the raid banner's size warning (kept for now, decided with the user).
