...
**Progress (2026-09-09):** steps 1-4 are built and compile, unplaytested — `FogOfWar` grid (2 m, explored + visible, eased RG16 mask), `VisionSource` on every unit and building (12 / 14 / 18 / 10 / 35 m), fog in the `OccluderCutout` and water shaders with fog-aware terrain and decor via `FogMaterials`, and `FogVisibility` hiding raiders (while unseen) and nodes and pickups (until explored). The F4 menu has a "Reveal map" toggle. The pickup half of the resource gate landed the same day (both pickup scans skip unexplored ground, found while fixing the forage stutter). Left: step 5's three gates (`ResourceAvailability` for nodes, the warrior engage filter, `GhostPlacer`), the raid banner's size warning (kept for now, decided with the user), and step 6's minimap.
