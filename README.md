...
That sweep also surfaced something the win rate hides, and it is the more useful result: **night 5 is not a close fight, it is a coin flip.** In 9 of 12 Eco runs the campfire finishes the last night at a *full* 200/200; in one it is destroyed outright. The cause is engagement geometry — `Warrior.searchRadius` is 50 and enemies spawn at 45, so every enemy is inside warrior range the instant it appears and all five warriors sprint out to fight at the map edge. Win that one field battle and nothing touches the base; lose it and nothing stands between the enemies and an undefended colony. It also explains why Turtle has lost every sweep it has entered: its wall ring sits ~9 units from the campfire, roughly 35 units *behind* the front line, so it converts its whole economy into fortifications the battle never reaches. A sweep of shorter warrior search radii (50 / 30 / 22 / 18 / 12) then refuted the obvious fix and pointed at a better one. Pulling warriors in did not let enemies reach the base — it made the shutout *more* complete: at radius 18 and 22 the campfire finished night 5 at a full 200/200 in **12 of 12** runs, against 7 of 12 at the shipping 50. The lever is not engagement *distance*, it is warrior *concentration*. At radius 50 each warrior independently chases the nearest enemy and fights alone, so the wave defeats a scattered line in detail; at 18 they hold together and win as a block. Turtle barely moved (0% → 8–17%), which says its problem was never the radius — it fields 1–3 warriors where Eco fields 5. The sweep files that produced all of this live in `SimSweeps/`.
