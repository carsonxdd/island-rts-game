...
3. Right-click a bush to strip it by hand for food and sticks — trees and rocks need a tool you do not have yet — then right-click the fire (or the ground beside it) to deposit what you carry and use the panel's **Research** tab: Woodcutting (3 sticks, 2 stone) teaches the colony to cut wood **and hands you the Stone Axe**, Quarrying does the same with the Stone Pick, Construction with the Mallet, and Spearcraft opens spears. Your character stands at the fire while the queue runs — walk away and it waits. Survivors come ashore by day while there is free housing — the campfire houses 3 — and the Colonists tab hands them the jobs you have researched
...
| **B** | Enter build mode |
| **1-6** | Select building (Hut, Wood Wall, Stone Wall, Watchtower, Workshop, Shipyard) |
| **F5 / F8 / F9** | Militia stance: Defensive / Offensive / Follow. The same buttons, plus the formation picker, sit in the bottom-right box once you have a warrior |
...
| **Right-click** | Command your character: fetch a stick / stone / crate, pick a bush by hand, work a tree / rock with the tool research gave you, deposit at the campfire or the ground beside it (and work its queue), work a Workshop bench, or walk there |
| **Left-click** | Open a building's panel — the campfire's tabs, or a Workshop's bench. The only gesture that opens UI; a right-click deposit never does |
...
Full reference — including how to drive the F3/F4 debug tools, the editor setup run order, and the older playtest checklists: [`docs/CONTROLS_AND_CHECKLIST.md`](docs/CONTROLS_AND_CHECKLIST.md). In-game: **Esc → Information** is the field guide (mechanics, research tree, recipes, raid roll), and in editor / development builds its **DEV** tab holds the playtest quests — a PLAYTEST tracker top-right lists the next ones, some tick themselves as you play, and **SUBMIT REPORT** copies a markdown report to the clipboard and writes it to `Playtests/`.
...
│   │   │   ├── Warrior/         # Engage (slots + kiting), Intercept (formations), Defend, Patrol, Follow, Retreat, Heal, Rearm
│   │   │   └── Enemy/           # EnemyAttack (single action + priority-based targeting)
│   │   ├── Shared/              # NavMesh throttling, stuck detection
│   │   └── Debug/               # F3 debug overlay
│   ├── UI/                      # Menus (runtime uGUI) + GameSettings, KeyBindings, Difficulty, PlayerHUD, CombatHUD, DevQuests
│   ├── GuardStance.cs           # Colony-wide militia order (Defensive / Offensive / Follow) + Formation.cs (Line / Wedge / Ring)
...
| **Your character** | Stays under your control all run — never a colonist, takes no housing, ignored by enemies. Right-click fetches pickups into a six-slot inventory (HUD strip at the bottom), picks a berry bush bare-handed, works a tree or rock **once research has handed over the matching tool** (axe for timber, pick for stone and ore) for its resource plus the sticks or chunks that come off it, and deposits at the fire (a click on the ground beside the fire counts): resources into the pool, sticks and stones into the campfire stockpile. Has HP that regenerates by the fire; at zero it is knocked out and stands back up ten seconds later. Losing the campfire is still the only defeat. |
| **Economy** | Wood, food, stone, metal. Workers gather autonomously (cutters, foragers, quarriers, miners). Carry capacity 5, 1/sec rate. Every tree on the island is choppable, shoreline palms included, and the big scattered boulders are quarryable — a boulder holds 70 stone against a rock node's 40. Stone is common and **metal is deliberately scarce**: roughly 70 rock nodes to 9 ore nodes on a standard island. Metal has no cost sink yet. |
...
| **Pickups** | Sticks and stones scattered on the island (+3 wood / +3 stone, and roughly a quarter are large and worth three times that), which trickle-respawn, plus one-time salvage: the wreck's cargo and the crates and barrels along the shore (+6 food / +5 wood). Workers of the matching job detour to collect nearby ones, and **idle colonists haul anything within ~35 m of the fire**. A hauled stick is worth its wood in the pool *and* one Stick in the stockpile, so what colonists gather pays for research and spears. Working a node sheds more of the same on the ground — stone throws off mostly large chunks. |
| **Colonists** | People are a pool, not a purchase. While there is free housing (campfire 3, hut 2), a survivor lands at the cove every ~20 s by day and walks to the home with room. The campfire panel hands idle colonists jobs and takes them back; it never spawns anyone. Idle colonists are the colony's utility labor: they build a waiting site, then work any bench with a queue, then repair, then tidy the beach, in the order the panel's four Priority sliders set (a slider at zero switches that work off). The Specialists rows pin a colonist to one trade — Builder, Crafter or Repairer — and nothing else; left-click an idle colonist in the world to open that tab. Warriors are idle colonists taking up arms (a Wooden Spear from the stockpile + 15F) and occupy housing; dismissing one returns the spear, losing one loses it. |
...
| **Research** | The tech tree, on the campfire panel's Research tab: Woodcutting / Foraging / Quarrying / Mining open the four jobs, Construction opens build mode and builders, Spearcraft opens spears and warriors, Crafting opens the Workshop, Storage Pits enlarges the stockpile. Each is researched once from sticks, stones and a little wood or stone; every locked control names the research that opens it. **A research that teaches a job also hands your character its tool** — Woodcutting gives the Stone Axe, Quarrying the Stone Pick, and so on — so learning and equipping are one step. The Workshop lists its own tier: Sharpened Tools (+30% gather), Sturdy Scaffolds (+50% build speed) and Racks and Baskets (more storage). |
| **Crafting** | Repeatable recipes at any bench: Wooden Spears (queue ×5) and, after Iron Work, Iron Spears go to the campfire stockpile and arm warriors; the Warriors row's picker chooses what the next recruit carries, and an idle warrior fetches a better spear on its own. The campfire and the Workshop are *stations* with a queue that only moves while someone stands at the bench — your character, or any idle colonist (or a pinned Crafter); the Workshop makes things at 2×. Costs are paid when an item finishes; a short entry waits for what is missing instead of failing. |
| **Storage** | Sticks, chunks, spears and tools live in the **campfire stockpile**, which holds **60 items** to begin with. Research raises it: Storage Pits (+40) at the campfire, Racks and Baskets (+80) at the Workshop. The Stockpile tab shows the room left and turns red when it is full; hauls that do not fit are lost, so a colony that wants to hoard has to invest in somewhere to put it. A dedicated **Warehouse building** is planned and will add its own capacity to the same colony total, so storage becomes something you place rather than only something you research. The four pooled resources (wood, food, stone, metal) are **not** capped. |
| **Combat** | The militia takes one colony-wide order from the campfire panel, the bottom-right militia box or F5 / F8 / F9: **Defensive** holds the colony and fights only what comes near the fire or the walls, **Offensive** advances on the raiders as a group and charges when they are close, **Follow** escorts your castaway. When they rally or escort they stand in a formation — Line (spearmen in front, archers behind), Wedge or Ring, picked by the stance or forced from the Formation row. Warriors on one raider come at it from different sides; archers keep their distance on Offensive and hold their rank on Defensive. Watchtowers buff nearby warrior damage 1.25x. There is no warrior cap: a warrior needs a bed and an idle colonist, so housing is the limit. |
| **Calendar** | 100s day / 50s night, 30 days to rescue (~75 minutes). Raids are not nightly: at each dawn a roll decides whether raiders land tonight — never before day 3, the chance climbing with every quiet night and guaranteed after five — and the HUD announces it for the whole day. A raid night lasts until the last raider is dead: dawn waits on the raid. |
| **Raids** | Size is fixed at the dawn roll: `2 + 0.4 x day + 0.08 x prosperity`, where prosperity counts colonists, warriors, buildings and stock on hand. A first raid on a bare colony is ~5 raiders; a day-20 colony with a garrison and walls sees ~16. Raiders land 0.4s apart from one direction so a raid arrives as one body, and the night holds until every one of them is dead. |
...
Latest: **The militia takes orders, and the night lasts the raid.** Warriors had one behaviour — chase the nearest raider — and five of them on one target queued up on the same path. They now take a colony-wide **stance**: Defensive holds the colony and engages only what threatens the fire or the walls; Offensive advances on the raiders as a group and breaks into the charge at twenty paces; Follow keeps a loose ring around your castaway and fights whatever reaches you. When the group rallies or escorts it stands in a **formation** — Line, Wedge or Ring, picked by the stance or forced — and warriors going for the same raider claim different bearings on it, so they come in from different sides. Archers kite on Offensive and hold their rank on Defensive, shooting as they step back. A militia box appears bottom-right once you have a warrior, with the stance and formation buttons; the stances are on F5 / F8 / F9 too, rebindable. The five-warrior cap is gone (housing is the limit), the campfire panel's sections fold, and a **job change sends the colonist back to the fire first** to drop their load and gear up before they head out as the new unit. Two fixes rode along: idle warriors used to freeze on "Patrolling" when they could not reach a post that sat on the fire's delivery edge (posts moved out, a stuck warrior picks another), and a cyan stripe of shallow water ran from the cove to the horizon on some seeds (the depth map's border is now forced to open ocean). And a raid night now lasts until the last raider is dead — the sky holds short of dawn and the calendar chip says so — instead of the survivors vanishing at sunrise.

Before that: **One flexible colonist, a field guide, and a playtest loop that reports back.** The Crafter job is gone as a job: an idle colonist now does whatever the colony needs — builds, then crafts, then repairs, then tidies — weighted by four sliders on the Colonists tab, with Builder / Crafter / Repairer specialists for when you want a hand pinned to one bench no matter what. Esc → Information is a six-tab field guide whose numbers (research tree, recipes, weapons, buildings, difficulty presets, the raid roll) are built from the game's own catalogs, so they cannot drift; the Story tab is empty until the story is written. And in editor and development builds there is a DEV tab of playtest quests: a tracker top-right lists the next ones, quests with a signal tick themselves as you play, each takes PASS / FAIL and a note, and SUBMIT REPORT copies a markdown report to the clipboard and writes it to `Playtests/`. Every new feature now ships with its batch of quests in `Assets/Resources/DevQuests.txt` instead of a checklist section in the docs.

Before that: **What falls on the ground gets picked up, and clicking the fire means two different things.** Workers already shed a stick or a chunk every few units off a tree, bush, rock or ore boulder, but almost nobody collected them: an idle colonist only looked within 35 metres of the campfire, and a job worker only notices a pickup within 22 metres of itself, so everything shed at a forest or a quarry across the island simply lay there. Idle colonists, builders and crafters now range 70 metres from the fire by day, and pull back to 30 after dusk so a colonist with island-wide reach is not out on the far shore when raiders land. Hand-harvesting with full hands no longer destroys the material either — the stick or chunk falls at your feet instead, under the same two-loose-per-node cap a worker's shedding respects. And the two campfire gestures are separate at last: right-click deposits (and gets on with whatever is queued at the bench), left-click on the fire itself opens its panel. Arriving to empty your hands used to throw the panel up every single trip.

Before that: **Polish from the first playtest of the research build.** Hovering anything used to wash it yellow; it now warms with a soft emissive glow taken from the object's own colour, which also stops the highlight fighting the tree fade for the same colour channel. Everything the player can pick up carries a low shimmer at rest so a stick in grass is findable at RTS zoom. Your character can work resource nodes by hand: right-click a bush for food and the sticks that come off it, a tree for wood and sticks, a rock or ore boulder for stone or metal and chunks — and a node that colonists are working sheds a stick or chunk onto the ground every few units, so the island keeps stocking the materials crafting runs on. There are more ground pickups, and about a quarter of them are large ones worth three. The HUD's resource chips open a breakdown under them — the stored total, everything in the stockpile filed under that chip, and the worker count — with Housing listing each building and who sleeps there, which is where families will land later; new item types appear in the right chip on their own. The editor-only "Day 1 - DAY (Time: 0.30)" label is replaced by a sky dial on the bar, a horizon arc the sun crosses by day and the moon by night. The tree fade was measuring the canopy, several metres of fronds, so a worker crossing a clearing blinked a row of trees; it tests the trunk now. Opening hints no longer hide behind the inventory strip. The larger asks from the same session — a collector job with a gather radius that grows with settlement tier and flags on top of it, processing chains for planks and blocks, families and breeding, livestock and farming — are designed in [`COLONY_EXPANSION_PLAN.md`](COLONY_EXPANSION_PLAN.md) and not yet built.

Before that: **Research teaches, crafting supplies.** The six "craft a tool once and the colony learns a job" recipes became a tech tree. Research is one-time and lives on the campfire panel's new Research tab — Woodcutting, Foraging, Quarrying, Construction, Spearcraft, Crafting, Mining at the fire, and the Workshop's two upgrades as Workshop-tier entries — and every job row, the warriors row and the B key now name the research they wait on. Recipes are repeatable and gated by research: the Wooden Spear is equipment, crafted ×5 at a go into the campfire stockpile, and a warrior is an idle colonist taking one plus 15 food — the old 10 wood is gone, and dismissing a warrior hands the spear back. Both the campfire and the Workshop are crafting *stations* with a queue that only moves while someone stands at the bench: pressing Craft or Research sends your character over, right-clicking the fire deposits and then works whatever is queued, and walking away simply pauses it. Costs are paid when an entry finishes, and an entry that comes up short waits at 100% with "Waiting for 2 Stick" rather than failing, so a queue of five spears cannot lose four because a colonist spent the wood. The old Workshop panel is gone; clicking a Workshop opens the same panel with its Craft · Research · Queue tabs. Two things surfaced on the way: the Warrior prefab has carried 25 damage and range 2 all along while the docs said 15 (the spear encodes the live numbers, so nothing changed in combat), and the balance sim can no longer be handed its unlocks for free — its policies research and queue spears like a player, and a new driver walks the character out to fetch sticks and stand at the bench. Slice 2 of [`RESEARCH_AND_DAYS_PLAN.md`](RESEARCH_AND_DAYS_PLAN.md); Slice 3 adds the Crafter job, the Workshop's 2× speed and the Iron Spear. Sweeps from before this change are not comparable.
